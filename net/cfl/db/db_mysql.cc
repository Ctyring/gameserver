 ...
 */
    MySQLResult::MySQLResult(mysqlx::RowResult result, int err, std::string errstr)
        ...
        // 从元数据构建列名 -> 索引映射
        int idx = 0;
        for (const auto &col: m_result.getColumns()) {
            std::string name = col.getColumnName();
            m_col_index[name] = idx++;
        }
    }
 ...
 */
    ...
    int MySQLResult::column_index(std::string_view name) const {
    ...
    [[nodiscard]] bool MySQLResult::is_null(std::string_view col_name) const {
        return is_null(column_index(col_name));
    }

    [[nodiscard]] int8_t MySQLResult::get_int8(std::string_view col_name) const {
        return get_int8(column_index(col_name));
    }

    [[nodiscard]] uint8_t MySQLResult::get_uint8(std::string_view col_name) const {
        return get_uint8(column_index(col_name));
    }

    [[nodiscard]] int16_t MySQLResult::get_int16(std::string_view col_name) const {
        return get_int16(column_index(col_name));
    }

    [[nodiscard]] uint16_t MySQLResult::get_uint16(std::string_view col_name) const {
        return get_uint16(column_index(col_name));
    }

    [[nodiscard]] int32_t MySQLResult::get_int32(std::string_view col_name) const {
        return get_int32(column_index(col_name));
    }

    [[nodiscard]] uint32_t MySQLResult::get_uint32(std::string_view col_name) const {
        return get_uint32(column_index(col_name));
    }

    [[nodiscard]] int64_t MySQLResult::get_int64(std::string_view col_name) const {
        return get_int64(column_index(col_name));
    }

    [[nodiscard]] uint64_t MySQLResult::get_uint64(std::string_view col_name) const {
        return get_uint64(column_index(col_name));
    }

    [[nodiscard]] float MySQLResult::get_float(std::string_view col_name) const {
        return get_float(column_index(col_name));
    }

    [[nodiscard]] double MySQLResult::get_double(std::string_view col_name) const {
        return get_double(column_index(col_name));
    }

    [[nodiscard]] std::string MySQLResult::get_string(std::string_view col_name) const {
    ...
    [[nodiscard]] std::time_t MySQLResult::get_time(std::string_view col_name) const {
...
//            spdlog::info("[MySQL] last_insert_id: {}", res.getAutoIncrementValue());
    ...
    SqlData::Ptr MySQL::query(std::string_view sql) {
        try {
//                spdlog::error("[MySQL] query begin: {}  dbname: {}", sql, m_dbname);
            auto res = m_session->sql(std::string(sql)).execute();
//                spdlog::error("[MySQL] query end: {}", res.count());
    ...
    MySQLTransaction::MySQLTransaction(std::shared_ptr<mysqlx::Session>
                                       sess, bool
                                       auto_commit)
...
//            spdlog::info("[MySQLTransaction] last_insert_id: {}", res.getAutoIncrementValue());
    ...
    MySQLStatement::MySQLStatement(std::shared_ptr<mysqlx::Session>
                                   sess, std::string
                                   stmt)
...
// ========== 绑定参数 ==========
    ...
    int MySQLStatement::bind(int idx, const void *data, int64_t size) {
        ...
        if (bound_params_.size() < static_cast<size_t>(idx)) bound_params_.resize(idx);
        bound_params_[idx - 1] = mysqlx::Value(
